...
import { hospitalListingAction ,hospitalDetailAction, currentPatientAction, addCategoryAction, categoryListingAction, currentOrderAction, userOrderAction} from './reducer';
...
export const fetchOrderByType = async (userId)=>{
  try{
    ...
    const response = await NetworkServices.get(`${Config.SERVER_URL}/get-order-by-type?id=${userId}&type=USER`);
    ...
    response.data.forEach((element,index)=>{
      ...
      response.data[index].orderStatus = element.orderStatus===0?'Active':'Finished';
    ...
    handleError(err);
    store.dispatch(userOrderAction.failed({ internalMessage: err.message, displayMessage: 'Error in fetchCategory' }));
  }
}